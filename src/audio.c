...
static int set_oss_params(int a, int fd, struct audio_s *pa)
{
	...
	if (devcaps & DSP_CAP_DUPLEX)
		printf("Full duplex record/playback.\n");
	if (devcaps & DSP_CAP_BATCH)
		printf("Device has some kind of internal buffers which may cause delays.\n");
	if (devcaps & ~(DSP_CAP_DUPLEX | DSP_CAP_BATCH))
		printf("Others...\n");
#endif
