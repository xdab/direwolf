...
float window(bp_window_t type, int size, int j)
{
	...
	case BP_WINDOW_BLACKMAN:
		w = 0.42659 - 0.49656 * cos((j * 2 * M_PI) / (size - 1)) + 0.076849 * cos((j * 4 * M_PI) / (size - 1));
		break;

	case BP_WINDOW_FLATTOP:
		w = 1.0 - 1.93 * cos((j * 2 * M_PI) / (size - 1)) + 1.29 * cos((j * 4 * M_PI) / (size - 1)) - 0.388 * cos((j * 6 * M_PI) / (size - 1)) + 0.028 * cos((j * 8 * M_PI) / (size - 1));
		break;
...
void gen_bandpass(float f1, float f2, float *bp_filter, int filter_size, bp_window_t wtype)
{
	...
	for (j = 0; j < filter_size; j++)
	{
		...
		{
			sinc = sin(2 * M_PI * f2 * (j - center)) / (M_PI * (j - center)) - sin(2 * M_PI * f1 * (j - center)) / (M_PI * (j - center));
		}
