...
#define GOOD_DEVICE(v, p) ((v == CMEDIA_VID && ((p >= CMEDIA_PID1_MIN && p <= CMEDIA_PID1_MAX) || p == CMEDIA_PID_CM108AH || p == CMEDIA_PID_CM108AH_alt || p == CMEDIA_PID_CM108B || p == CMEDIA_PID_CM119A || p == CMEDIA_PID_CM119B)) || \
...
void cm108_find_ptt(char *output_audio_device, char *ptt_device, int ptt_device_size)
{
	...
	if (good_devices == 0)
		return; // None found - caller will print a message.

	if (good_devices == 1)
		return; // Success - Only one candidate device.
